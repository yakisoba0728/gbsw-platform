...
기록만 해 두고 이번 범위에서는 고치지 않은 것들.

- **명단 확정 화면이 개인정보를 DOM에 담아 둔다 (M14).** `/admin/students/import`의
  확정 단계는 서버가 다시 검증할 값(최대 수백 명분의 명단)을 hidden input에
  JSON으로 실어 보낸다. 서버가 항상 재검증하므로 신뢰 문제는 아니지만, 그
  값이 세션 복원·오류 리포팅 도구의 DOM 스냅샷에 함께 딸려 나갈 수 있다는
  사실은 기억해 둔다. 그런 도구를 붙일 때는 이 화면을 마스킹 대상에 넣는다.
- **감사로그 화면이 접속 IP를 보여준다 (M18).** IP도 개인정보다. 지금은
  보존 기간 정책이 없다 — 얼마나 오래 남길지, 언제 IP만 지우고 나머지
  기록은 남길지를 업무 모듈이 늘어나기 전에 정해야 한다.
