...
> **현재 상태 (2026-09-02):** 이 문서의 최초 범위를 확장해 가입 시 이메일과
> 전화번호를 모두 수집하고 각각 실제 인증코드 발송·확인을 거친다. 아래 「범위 밖」은
> 2026-08-12 설계 당시의 기록이며 현재 동작을 설명하지 않는다. 운영 기준은
> `CLAUDE.md`의 「가입 인증은 실제 발송과 코드 확인을 거친다」와
> `src/modules/verification/`을 따른다.
