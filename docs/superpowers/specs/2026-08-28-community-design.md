...
- ~~**마크다운도 서식 편집기도 없다. 본문은 평문이다.**~~ **뒤집었다 (2026-08-28).**
  아래 「결정 4」에 근거와 안전장치를 적었다.
...
## 결정 4 — 마크다운을 넣는다 (2026-08-28, 처음 결정을 뒤집음)

처음에는 「본문은 평문」이었다. 근거는 **서식을 넣는 순간 HTML 살균이 이 모듈에서
가장 위험한 코드가 된다**는 것이었고, 그 위험 자체는 지금도 사실이다. 뒤집은
이유는 공지·안내가 제목과 목록과 표 없이는 읽히지 않아서다.

**위험을 없애는 대신 살균을 두 겹으로 세웠다** (`components/ui/markdown.tsx`).

1. **날 HTML을 애초에 파싱하지 않는다.** `rehype-raw`를 쓰지 않으므로 `<script>`는
   태그가 아니라 아무것도 아닌 것이 된다(안의 글자는 남는다). **살균기가 뚫려도
   통과시킬 HTML 자체가 없다는 것이 이 설계의 핵심이다.**
2. **허용 목록으로 한 번 더 거른다** (`rehype-sanitize`). 언젠가 누가 ①을
   되돌리더라도 이 그물이 남는다.

목록에서 뺀 것: `img`·`input`과 프레임·미디어 계열. 이미지를 빼는 이유는 마크다운으로
아무 주소나 걸 수 있게 되면 게시판이 바깥 서버에 조회를 보내는 자리가 되어서다 —
사진은 첨부로 올리면 글에 그려진다. 주소는 `http`·`https`·`mailto`만 남고, 링크는
새 탭에서 `rel="noopener noreferrer nofollow"`로 열린다.

**코드로 감싼 HTML은 글자 그대로 보인다.** 소프트웨어고등학교 게시판이라 코드
얘기를 쓰는 자리가 반드시 필요하다.

**댓글은 평문 그대로다.** 짧은 대화라 서식이 필요 없고, 주소는
`components/ui/plain-text.tsx`가 링크로 만든다.

약속들은 `tests/components/ui/markdown.test.tsx`가 붙든다 — 서버가 실제로
내보내는 HTML 문자열을 검사하므로 살균 뒤의 결과를 본다.
...
- 상한: **파일당 20MB, 글당 5개.** 이 값을 올리면 프록시 본문 상한과 앱 컨테이너의 `mem_limit`이 함께 움직인다 — 업로드 한 건이 파일 크기의 서너 배를 잠깐 메모리에 든다.
