...
QR 내용 = https://<호스트>/p/<passId>.<base64url(mac[0..12])>
...
- **`START_IN_PAST`는 학생 신청에만 걸리고, 10분 유예가 있다.** 14:00 외출을 13:59에
  적다가 14:01에 내면 그것은 실수가 아니다. 교사 직접 부여는 `startAt`을 입력받지
  않으므로(항상 지금) 이 오류가 나올 자리가 없다.
...
"pass:verify"    → ["STUDENT"]   // 학생 + 교사. 학부모는 제외
...
`pass:verify`에서 학부모를 뺀 근거: 정문·기숙사에서 확인하는 사람은 교직원이거나
자치위원 학생이다. 학부모에게 남의 자녀 출입증을 열 자리를 줄 이유가 없다.

**그래서 학부모가 자녀의 QR을 찍으면 판정이 안 나온다.** 이건 의도한 것이되, 거기서
빈 권한 오류를 띄우지는 않는다 — `/p/[token]`은 학부모에게
「출입증 확인은 학생·교사 계정에서만 됩니다」와 함께 **자녀의 출입증 상세로 가는 링크**를
준다. 학부모가 알고 싶은 것은 「지금 이게 진짜인가」가 아니라 「우리 애가 어디 갔나」다.
...
| `/pass/scan` | `(app)` | 카메라 스캐너 + 6자리 입력칸 |
| `/p/[token]` | **`(app)` 밖** | 검증 결과 |

**`/p/[token]`을 `(app)` 밖에 두는 이유는 로그인 후 돌아오기 때문이다.**
...
페이지가 직접 `getSessionUser()`를 부르고 없을 때
`redirect(`/login?next=/p/${token}`)`을 할 수 있다.
