...
| 수명 | 20초(직전 창까지 40초) | **그대로** |
| 서명 대상 | `passId:step` | `studentProfileId:step` |
| HKDF info | `gbsw-pass-qr-v1` | `gbsw-student-qr-v2` |
...
**바뀐 것은 코드가 붙는 곳뿐이다.** 20초 회전은 본문 그대로 남는다 —
`gbsw-student-qr-v2` 키로 `studentProfileId:step`에 서명한다.

고정 코드로 잠깐 갔다가 되돌렸다. 그 판단의 근거는 「이 코드는 허가가 아니라
신원이니 사진이 새도 남의 이름이 뜰 뿐」이었는데, 회전을 되살리면 그 논리를
쓰지 않고도 사진 유출이 막힌다. `StudentProfile`에 회전용 열은 여전히 두지
않는다 — 스텝이 시간에서 나오므로 저장할 것이 없다.
