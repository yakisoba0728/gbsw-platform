...
> ## ✅ 현재 상태 (2026-09-02)
>
> 명단 반영의 소프트 삭제가 다시 도입됐다. 현재 `roster.repo.applyRoster`는 명단에서
> 빠진 사용자를 `INACTIVE`/`deletedAt`으로 표시하고 로그인 세션과 미사용 초대만
> 정리하며, User·StudentProfile·상벌점·학부모 연결 기록은 보존한다. 같은 학생코드가
> 다시 들어오면 기존 프로필과 계정을 활성 상태로 복구한다.
>
> 바로 아래 2026-08-25 경고는 당시의 물리 삭제 구현을 기록한 역사 스냅샷이며 이제
> 현재 동작이 아니다. 본문의 목표와 현재 구현은 다시 일치하지만, 체크리스트를 새 작업
> 지시로 실행하지 말고 현재 코드와 테스트를 기준으로 판단한다.
