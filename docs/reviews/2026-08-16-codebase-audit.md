...
>
> **결정이 필요한 것 (코드로 못 정한다)**
> - ~~`DEMERIT_THRESHOLDS`의 20/30은 임시값이다~~ — **코드에서 뺐다.** 관리자가
>   `/admin/settings`에서 트랙별로 정하고, 바꾸면 감사로그에 남는다. 코드의 20/30은
>   "아직 한 번도 정하지 않았을 때"의 폴백으로만 남는다 — 그래서 학교가 실제로 정했는지
>   아닌지가 화면에 드러난다. 즉 이 항목은 **답을 정한 게 아니라 답을 정할 자리를 만든** 것이다.
> - ~~대량 삭제 임계 비율 10%~~ — **정해서 없앴다.** 임계를 조정하는 대신 지워,
>   삭제 대상이 1명이라도 있으면 인원 수 직접 입력을 항상 요구한다. 임계는 학교가
>   클수록 함께 커져서 정작 큰 사고(한 반이 통째로 빠진 파일)를 놓치는 구조였다.
