...
## 부록보다 먼저 — 도메인이 없으면 카메라 스캔이 안 된다

`/scan`의 사이트 안 스캐너는 브라우저의 카메라 API를 쓰는데, **`http`로 열면
브라우저가 그 API를 통째로 감춘다**(secure context가 아니다). IP 주소만으로 띄운
테스트 서버에서는 이 화면이 「http로 열려 있어 카메라를 쓸 수 없습니다」로 떨어진다.

**나머지는 전부 동작한다.** 폰 기본 카메라로 QR을 찍으면 `/scan?c=…`가 열려 같은
판정이 나오므로, 정문에서 쓰는 주된 경로는 http에서도 살아 있다. 연속 스캔만 못 한다.

도메인 + 인증서가 붙으면 그때 `BETTER_AUTH_URL`도 `https://`로 함께 바꾼다
(어긋나면 로그아웃이 `INVALID_ORIGIN`으로 실패한다).
...
- 토큰은 **서버 콘솔에만** 나온다 — 서버에 접근할 수 있는 사람만 최초 교사 계정을 만든다
