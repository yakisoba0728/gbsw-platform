...
**코드 행은 발송 전에 「예약」으로 태어난다** — `expiresAt = createdAt`이 그 표식이고,
발송이 성공해야 활성화되어 만료 시각이 뒤로 밀린다. 예약 행을 지우는 청소는 반드시
**유예**(`RESERVATION_GRACE_MS`, 외부 발송 상한 10초보다 넉넉한 60초)를 지난 것만
건드린다. 유예 없이 「지금」을 기준으로 지우면 **발송 중인 행을 다음 요청이 지우고,
지워진 행은 대상·초대·IP 세 한도의 계산에서 모두 사라져 발송 예산이 무의미해진다.**
발송보다 빠르게 이어 부르기만 하면 문자를 무제한으로 보낼 수 있었던 실제 결함이다.

**발송 한도의 잠금 순서는 대상 → 초대 → IP 하나뿐이다**(`lockSendRateLimitBuckets`).
세는 것과 넣는 것을 직렬화하려면 세 축 모두를 잠가야 하고, 순서를 고정해야 서로 다른
조합의 요청이 교착하지 않는다. 새 축을 더할 때도 이 함수 한 곳에서 순서를 정한다.
...
- **로그인의 허용 오리진은 `BETTER_AUTH_URL` 하나다.** 앱이 `127.0.0.1`에만 묶여
  공개 주소를 헤더로 알 수 없으므로 `host`·`x-forwarded-host`에서 유도하지 않는다.
  **그 값이 실제 접속 주소와 어긋나면 로그인이 전부 403이 된다**(fail-closed).
  개발에서만 요청 자신의 오리진도 허용한다 — `.localhost` 하위 도메인으로 역할을
  나눠 띄우는 방식을 살려 두기 위해서다.
- **최초 관리자 생성은 advisory lock으로 직렬화한다.** 콘솔 토큰은 프로세스 메모리라
  인스턴스가 둘이면 서로를 보지 못한다. 잠금을 잡은 뒤 트랜잭션 **안에서** 사용자 수를
  다시 센다. 저장소의 유일한 advisory lock이며, 새로 쓸 때는 다른 문자열에서 키를 뽑는다
...
- **첨부 다운로드는 바이트를 버퍼에 올리지 않는다.** `getDownload`는 좌표
  (`storageKey`·`storedAt`·`size`)만 주고 라우트가 `openAttachment`로 디스크에서
  응답으로 흘려보낸다. 파일당 20MB이므로 버퍼로 받으면 동시 내려받기 수만큼 그
  크기가 힙에 쌓인다. **`size`는 DB의 값이 아니라 `stat`이 읽은 실제 크기다** —
  Content-Range·Content-Length가 응답하는 파일과 어긋나면 안 되고, 그 `stat`이
  없는 파일을 ENOENT로 걸러 404를 만든다. Range 해석은 `parseRangeHeader` 순수
  함수가 하고 **알아듣지 못하는 형태는 전부 전체 응답으로 떨어뜨린다** — Range는
  서버가 무시해도 되는 요청이라 애매한 값에 206을 붙이는 쪽이 위험하다.
- **목록을 전부 내려주는 화면을 새로 만들지 않는다.** 출입증 내역 내보내기는
  `PASS_HISTORY_EXPORT_MAX_ROWS`(5,000)를 넘으면 기간을 좁히라고 답한다. 상한이
  없으면 조회·시트 생성·전송이 한 요청에 몰려 그동안 다른 요청이 밀린다.
