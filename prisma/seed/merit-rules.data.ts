...
export const SCHOOL_RULES: SeedRule[] = [
  ...
  // ── 상쇄점 ──────────────────────────────────────────────────
  // 원본 표에서 "상쇄점"으로 분류된 유일한 항목이다. 이 시스템의 종류는
  // 상점/벌점 둘뿐이라 상점으로 넣되, 성격이 다르다는 사실을 설명에 남긴다 —
  // 일상적으로 주는 상점이 아니라 위원회 의결로만 나가는 항목이다.
  { track: "SCHOOL", kind: "MERIT", category: "선도관리 위원회", points: 60,
    description: "상쇄점 — 선도관리위원회 의결로만 부여한다. 일반 상점과 성격이 다르다.",
